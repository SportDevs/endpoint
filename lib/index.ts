...
function orderTable(): OrderObject {
  ...
  const obj = {
    ...
    toString() {
      const arr = [];
      for (const [key, value] of Object.entries(sortedKeys).filter(
        (x) => !Object.values(x[1]).every((x) => !x)
      )) {
        let str = key;
        if (value.ascending) {
          str += ".asc";
...
type EndpointParam = [string, string | undefined];
type EndpointParams = EndpointParam[];

type LogicalOperator = (
  fn: (obj: EndpointObject) => EndpointObject
) => EndpointObject;
  ...
  const obj: EndpointObject = {
    ...
    toString(complex = false) {
      ...
      return (
        ...
        "?" +
        params
          .map((x) => (x[1] === undefined ? x[0] : `${x[0]}=${x[1]}`))
          .join("&")
      );
    },
  };
